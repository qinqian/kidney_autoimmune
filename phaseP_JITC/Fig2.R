...
p3=FeaturePlot_scCustom(niche.cna, features = c('cna_ncorrs_fdr10'), raster=T, raster.dpi=c(100, 100))[[1]] + #
...
p3=FeaturePlot_scCustom(niche.cna, features = c('VCAM1'), raster=T, raster.dpi=c(100, 100))[[1]] +
    scale_color_gradient2(high = "#de2d26", mid = "white", low = "#2c7fb8", midpoint = 0,  guide = guide_colorbar(direction = "vertical"))+
...
fig2b = DimPlot_scCustom(niche.merge, group.by='niche_label', raster.dpi=c(100, 100), raster=T) + scale_color_manual(values=niche_cols)
...
p20 = ggplot() +
    rasterise(geom_sf(data = as_tibble(imm.niche@meta.data, niche_label=imm.niche$niche_label_fine)%>% filter(patient_id==case1),
            aes(geometry = shape, fill = niche_label), color = NA), dpi=156) + theme_bw(base_size=12) + scale_fill_manual(values=niche_cols)+ theme(legend.position = 'none')+
...
p51 = ggplot() + 
    rasterise(geom_sf(data = df1,
            aes(geometry = shape, fill = !!sym(feature2)), color = NA), dpi=156) + theme_bw(base_size=12) +
...
ggsave('Fig2_v4.pdf', width=18.5, height=21.5)
