...
sceasy::convertFormat(kidn_seu_obj$seur, from="seurat", to="anndata", outFile='shapes_seur_kidney_v5_miles.h5ad', main_layer='data')

t_nk.markers = FindAllMarkers(t_nk, only.pos = T, group.by="cluster_name")
...
png("sanity_check_t_nk.png", width=1800, height=1200)
print(DoHeatmap(object = t_nk, features = unique(seq.top10$gene), label = TRUE, group.by='cluster_name',
                slot="data")+NoLegend())
dev.off()

png("sanity_check_t_nk_dot.png", width=1800, height=1200)
DotPlot(object = t_nk, features = unique(seq.top10$gene), group.by='cluster_name')+theme(axis.text.x=element_text(size=12, angle=90))
...
write_tsv(as.data.frame(pred.cnts), "T_kidney_lupus_pred.cnts")
...
png("kidney_T_heatmap.png", width=1200, height=600)
plotScoreHeatmap(pred.cnts)
dev.off()

## more diagnostics of delta
png("kidney_T_deltadiag.png", width=1200, height=600)
plotDeltaDistribution(pred.cnts)
dev.off()

png("kidney_T_umap.png", width=1600, height=1200)
...
p4 = DimPlot(t_nk, group.by="cluster_name", label=T) + NoLegend()
print(p1 + p2 + p3 + p4)
dev.off()
...
png("singleR_kidney_T_seqmarker_dotplot.png", width=1600, height=800)
