...
xen <- readRDS(input_data[1])
print(table(xen$cell_type))
print('-------')

xen.T <- subset(xen, subset = grepl(celltypeA, cell_type, ignore.case=TRUE))
...
print('A-------')
sc.de <- getDE(subset(xen.T, subset=tech == 'sc'), "cell_type", top_n=500)
print('B-------')

markers <- unique(sc.de)
xen.T.sub = xen.T[markers, ]

print(mean(xen.T.sub@meta.data$nCount_RNA))
print(head(xen.T.sub@meta.data))
merged.T = CreateSeuratObject(LayerData(xen.T.sub[['RNA']], "counts"), meta.data=xen.T.sub@meta.data %>% select(orig.ident, tech, cell_type, celltype))
print(head(merged.T@meta.data))
print('C-------')
print(mean(merged.T@meta.data$nCount_RNA))
...
median.xen = median(colSums(LayerData(xen.T.sub[['RNA']], "counts")[, xen.T.sub@meta.data$tech == 'xenium']))
median.sc = median(colSums(LayerData(xen.T.sub[['RNA']], "counts")[, xen.T.sub@meta.data$tech == 'sc']))
print(median.sc)
print(median.xen)
...
DimPlot(merged.T.proc, split.by='tech', group.by='cell_type', reduction='harmony')+get_theme(size=11)
DimPlot(merged.T.proc, split.by='tech', group.by='cell_type', reduction='umap')+get_theme(size=11)
scCustomize::DimPlot_scCustom(merged.T.proc, reduction='umap')+get_theme(size=11)
dev.off()

de.integrate = getDE(merged.T.proc, 'seurat_clusters')

pdf(glue("kpmp/{output}_{label}_Rplots_integrate_clusters.pdf"), width=18)
DotPlot(merged.T.proc, de.integrate)+get_theme(size=11)
dev.off()
