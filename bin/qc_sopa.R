...
adata@meta.data$orig.ident = basename(adata@meta.data$orig.ident)

png(glue("qc/{args$output}_orig_seg_qc.png"), width=1100, height=760)
#VlnPlot(adata, features = c("nFeature_Xenium", "nCount_Xenium"), group.by='orig.ident', ncol = 1, pt.size=0)
Stacked_VlnPlot(adata, features = c("nFeature_Xenium", "nCount_Xenium"), group.by='orig.ident', pt.size=0, x_lab_rotate = TRUE)
#p1 = ggplot(adata@meta.data, aes(orig.ident, nFeature_Xenium)) +
#    geom_violin(draw_quantiles = c(0.02, 0.98))+get_theme(12, 45)
#p2 = ggplot(adata@meta.data, aes(orig.ident, nCount_Xenium)) +
#    geom_violin(draw_quantiles = c(0.02, 0.98))+get_theme(12, 45)
#print(p1+p2+plot_layout(heights=c(1, 1), widths = c(1, 1), guides = "collect")& theme(legend.position = "none"))
dev.off()
