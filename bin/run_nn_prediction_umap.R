...
x.tr = xen.int[["humap"]]@cell.embeddings[which(xen.int$tech == 'sc'), ]
y.tr = factor(xen.int$cell_type[which(xen.int$tech == 'sc')])
x.te = xen.int[["humap"]]@cell.embeddings[which(xen.int$tech == 'xenium'), ]
yhat <- fastknn(x.tr, y.tr, x.te, k = 10)
xen.int$cell_type[which(xen.int$tech == 'xenium')] = as.character(yhat$class)

x.tr = xen.int[["humap"]]@cell.embeddings[which(xen.int$tech == 'sc'), ]
y.tr = factor(xen.int$celltype[which(xen.int$tech == 'sc')])
x.te = xen.int[["humap"]]@cell.embeddings[which(xen.int$tech == 'xenium'), ]
yhat <- fastknn(x.tr, y.tr, x.te, k = 10)
xen.int$celltype[which(xen.int$tech == 'xenium')] = as.character(yhat$class)
...
saveRDS(xen.int, glue("output/{args$output}_umapnn_labels_umap.rds"))
