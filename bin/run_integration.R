...
if (length(input_data) == 3) {
   ...
   myles.skin = readRDS(input_data[1])
   sc = readRDS(input_data[2])
   merged.obj = merge(myles.skin, sc)
...
print(colnames(merged.obj@meta.data))
merged.obj@meta.data = merged.obj@meta.data %>% mutate(sample_ids=ifelse(is.na(SampleID), sample_id, SampleID))
...
pdf(glue("{args$output}_annt_um_split.pdf"), width=18, height=12)
...
print(p5)
dev.off()
saveRDS(merged.seurat, glue('{args$output}_merged_annotated_sc.rds'))


pdf(glue("{args$output}_annt_um.pdf"), width=11, height=6)
