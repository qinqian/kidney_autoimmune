...
do_bar_chart <- function(input, out_path, threads, myparam) {
    ...
    print(head(data))
    celltype <- data %>% select(preannotation) %>% distinct() %>% pull()

    pdf(out_path[["fig"]], width=12, height=5)
    for (cell in celltype) {
	    print(cell)
	    print(data %>% filter(preannotation==cell) )
	    print(dim(data %>% filter(preannotation==cell) ))
	    print(ggplot(data=data %>% filter(preannotation==cell)) +
	    ...
	    print(data %>% filter(preannotation==cell) %>% tidyplot(x=distance, y=probability, color=group) %>%
                  add_mean_line() %>%
                  add_mean_dot() %>%
                  add_sem_ribbon()+ggtitle(cell))
	    print(data %>% filter(preannotation==cell) %>% tidyplot(x = group, y=probability, color=group) %>% 
